    ...
    fun get(code: LlmPromptCode): String =
        when (code) {
            LlmPromptCode.CONSULTING_SYSTEM_INVESTMENT_SAJU -> """
                ...
                - STRONG(신강): 주관과 확신이 매우 강한 타입. 에너지를 외부로 분산해야 하므로 한 종목에 묶어두기보다 포트폴리오를 다각화하여 순환시키라고 조언하라. 자신감 과열 시 손절 라인을 기계적으로 지키는 것이 핵심이다.
                ...
                운세는 투자의 참고 지표일 뿐임을 이성적 어조로 명시한다. JSON만 반환해라.
            """.trimIndent()
            LlmPromptCode.CONSULTING_SYSTEM_INVESTMENT_TAROT -> """
                ...
                [타로 해석 제약] 모든 카드는 정방향(Upright)으로만 추출된다. 긍정 카드는 온전히 긍정 에너지로, 부정 카드(예: 소드 3, 타워 등)는 지연·이중 부정 없이 강력한 경고 및 리스크 시그널로 직관적으로 해석하라. JSON만 반환해라.
            """.trimIndent()
            LlmPromptCode.CONSULTING_SYSTEM_INVESTMENT_ZODIAC -> """
                ...
                [동적 기운 융합 규칙] 사용자의 고정된 태양궁(signals.zodiac.signKo) 성향에, 오늘 하루 시장에 영향을 미치는 '요일 지배 행성(signals.zodiac.todayPlanetRule)'의 단기 기운을 결합하여 해석하라. 사용자의 타고난 투자 기질이 오늘의 행성 기운(예: 수성의 정보 민감성, 화성의 공격성 등)과 만났을 때 발생할 수 있는 특별한 심리 변화나 투자 타이밍을 포착하여 텍스트를 다채롭게 구성하라. JSON만 반환해라.
            """.trimIndent()
            LlmPromptCode.CONSULTING_SYSTEM_INVESTMENT_ALL -> """
                ...
                overall_summary에 오늘의 투자 기상도(맑음/흐림/비)와 권장 자산 대 현금 비중(예: 자산 60%/현금 40%)을 반드시 명시하라. JSON만 반환해라.
            """.trimIndent()
