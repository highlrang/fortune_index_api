    ...
    override fun run(args: ApplicationArguments?) {
        ...
        val templates = listOf(
            seedTemplate(
                ...
                사주 상징은 장황하게 설명하지 말고 사용자의 주식 투자 성향, 보유 기준, 진입 속도, 손실 한도 점검으로 번역해라.
                """.trimIndent(),
                now = now
            ),
            seedTemplate(
                ...
                카드 상징은 장황하게 설명하지 말고 사용자의 지금 감정, 충동, 확신 욕구, 진입 전 체크포인트로 번역해라.
                """.trimIndent(),
                now = now
            ),
            seedTemplate(
                ...
                별자리 상징은 장황하게 설명하지 말고 오늘의 판단 분위기, 속도 조절, 보류 또는 유지 기준으로 번역해라.
                """.trimIndent(),
                now = now
            ),
            seedTemplate(
                ...
                각 상징은 장황하게 설명하지 말고 주식 투자 성향, 현재 감정, 오늘의 판단 기준으로 나눠 번역해라.
                """.trimIndent(),
                now = now
            ),
            seedTemplate(code = LlmPromptCode.CONSULTING_QUESTION_INVESTMENT_SAJU, content = "내 사주 흐름을 바탕으로 오늘 주식 투자 판단을 짧게 봐줘.", now = now),
            seedTemplate(code = LlmPromptCode.CONSULTING_QUESTION_INVESTMENT_TAROT, content = "타로로 지금 투자 심리와 오늘의 판단 기준을 짧게 봐줘.", now = now),
            seedTemplate(code = LlmPromptCode.CONSULTING_QUESTION_INVESTMENT_ZODIAC, content = "별자리로 오늘 주식 투자 판단 분위기를 짧게 봐줘.", now = now),
            seedTemplate(code = LlmPromptCode.CONSULTING_QUESTION_INVESTMENT_ALL, content = "사주, 타로, 별자리로 오늘 주식 투자 심리와 판단 기준을 짧게 봐줘.", now = now)
        )
