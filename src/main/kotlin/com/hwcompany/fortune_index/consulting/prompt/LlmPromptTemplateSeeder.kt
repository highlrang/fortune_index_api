    ...
    override fun run(args: ApplicationArguments?) {
        ...
        val templates = listOf(
            ...
            seedTemplate(
                ...
                content = """
                너는 사주와 타로를 함께 읽어 오늘의 돈 흐름과 마음 상태를 풀어 주는 안내자다.
                어려운 투자 용어, 이름이나 코드, 무엇을 사거나 팔라는 표현은 절대 쓰지 마라.
            ...
            seedTemplate(code = LlmPromptCode.CONSULTING_QUESTION_STOCK_SAJU, content = "내 사주 흐름을 바탕으로 오늘의 재물 기운을 읽어줘.", now = now),
            seedTemplate(code = LlmPromptCode.CONSULTING_QUESTION_STOCK_TAROT, content = "타로 3장으로 지금 마음 흐름과 오늘의 조언을 읽어줘.", now = now),
            seedTemplate(code = LlmPromptCode.CONSULTING_QUESTION_STOCK_ALL, content = "사주와 타로를 함께 보고 재물 운세와 마음 상태를 읽어줘.", now = now)
        )
